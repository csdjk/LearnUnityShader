// 基于距离的曲面细分
Shader "lcl/Tessell/DistanceBasedTessell" {
    Properties {
        _Color ("Color", Color) = (1, 1, 1, 1)
        [PowerSlider(3.0)]_TessellationFactors("_TessellationFactors",Range(0,60)) = 1
        _DistanceRange("DistanceRange", Vector) = (1, 1, 1, 1)
    }
    SubShader {
        Pass { 
            ...
            //有些硬件不支持曲面细分着色器，定义了该宏就能够在不支持的硬件上不会变粉，也不会报错
                ...
                float4 _DistanceRange;

                //InputPatch<v2t, 3> 表示输入的数据有3个v2t结构体，即对于三角形三个顶点
                UnityTessellationFactors hsConstFunc(InputPatch<v2t, 3> v) {
                    //定义曲面细分的参数
                    UnityTessellationFactors o;
                    float minDist = _DistanceRange.x;
                    float maxDist = _DistanceRange.y;
                    float4 factor = UnityDistanceBasedTess(v[0].vertex, v[0].vertex, v[0].vertex, minDist, maxDist, _TessellationFactors);
                    o.edge[0] = factor.x; 
                    o.edge[1] = factor.y; 
                    o.edge[2] = factor.z;
