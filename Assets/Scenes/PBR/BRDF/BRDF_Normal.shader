Shader "lcl/BRDF/BRDF_Normal"
{
	Properties
	{
		...
		_Metallic ("Metallic", Range(0, 10)) = 1.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			// F - 菲涅尔函数（Fresnel Equation）
			float3 FresnelSchlick(float3 H,float3 V,float3 F0) {
				float cosTheta = saturate(dot(H, V));
				return F0 + (1.0 - F0)*pow(1.0 - cosTheta, 5.0);
			}

			float3 fresnelSchlickRoughness(float3 N,float3 V, float3 F0, float roughness) {
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float3 iblKS = fresnelSchlickRoughness(N, V, F0, roughness);
