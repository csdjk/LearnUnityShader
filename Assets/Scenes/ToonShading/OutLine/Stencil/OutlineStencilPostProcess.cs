...
public class OutlineStencilPostProcess : MonoBehaviour
{
    ...
    private RenderTexture stencilBufferTex;
    ...
    void Start()
    {
        ...
        stencilBufferTex = new RenderTexture(Screen.width, Screen.height, 24);

        OutlinePostProcessByStencilMat.SetTexture("_StencilBufferToColor", stencilBufferTex);
    }
    ...
    void OnPostRender()
    {
        ...
        Graphics.SetRenderTarget(stencilBufferTex);
        ...
        Graphics.SetRenderTarget(stencilBufferTex.colorBuffer, cameraRenderTexture.depthBuffer);
    ...
    private void OnGUI() {
        GUI.DrawTexture(new Rect(0,0,256,256),stencilBufferTex,ScaleMode.ScaleToFit,false,1);
    }
}